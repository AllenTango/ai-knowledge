...
def _call_worker(task: str, previous_feedback: str = "") -> str:
    ...
    logger.info(f"Worker 输出 {len(text)} 字符，消耗 {usage.total_tokens} tokens")
    return text
...
def _call_supervisor(task: str, output: str) -> dict:
    ...
    logger.info(f"Supervisor 输出 {len(text)} 字符，消耗 {usage.total_tokens} tokens")
...
def supervisor(task: str, max_retries: int = 3) -> dict[str, Any]:
    ...
    for attempt in range(1, max_retries + 1):
        ...
        logger.info(f"审核结果: passed={passed}, score={score}, " f"feedback={feedback[:50]}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Worker-Supervisor 质量审核循环")
