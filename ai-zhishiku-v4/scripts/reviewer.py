...
Usage:
    python -m scripts.reviewer
    python -m scripts.reviewer --verbose
"""
...
def review_article(article: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": f"总分 {total_score}/100，等级 {grade}，" f"判定: {verdict}",
        "issues": issues,
...
if __name__ == "__main__":
    ...
    for r in results:
        print(f"  [{r['verdict']}] {r.get('article_title', '?')} " f"- {r['score']}/100 ({r['grade']})")
