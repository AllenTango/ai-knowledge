...
from scripts.collector import fetch_rss_sources
from workflows.state import KBState

# Security imports
from scripts.security import sanitize_input, filter_output
...
def _merge_cost(
    ...
    ct["providers"][provider]["tokens"] = ct["providers"][provider].get("tokens", 0) + tokens
    return ct
...
def collect_node(state: KBState) -> dict:
    ...
    url = "https://api.github.com/search/repositories" f"?q={encoded_query}&sort=stars&order=desc&per_page=10"
...
def analyze_node(state: KBState) -> dict:
    ...
    from scripts.model_client import chat, check_llm_available
...
def organize_node(state: KBState) -> dict:
        ...
        if check_llm_available():
            ...
            for item in deduped:
                ...
                try:
                    ...
                    cost_tracker = _merge_cost(cost_tracker, usage.total_tokens, provider, "organize")
                except Exception as e:
    ...
    for item in deduped:
        ...
        article = {
            ...
            "score": round(item.get("score", 0) * 10, 1),  # 转换为 0.0~10.0 标准
            "reviewer": None,
            "reviewed_at": None,
            "published_to": [],
            "retry_count": 0,
...
def review_node(state: KBState) -> dict[str, Any]:
    ...
    from scripts.model_client import chat_json, check_llm_available
    ...
    for item in to_review:
        review = next((r for r in reviews if r.get("id") == item.get("id")), None)
        if review:
            logger.info(f"  {item.get('title', '?')[:35]} " f"总分={review.get('weighted_score', '?')} " f"passed={review.get('passed')}")
    ...
    if not all_passed:
        ...
        for r in failed:
            ...
            feedback_parts.append(
                f"[{r.get('id', '?')}] 加权总分={r.get('weighted_score', 0)}，" f"低分维度: {', '.join(dim_list) if dim_list else '整体偏低'}"
            )
        feedback = "; ".join(feedback_parts)

    logger.info(f"审核完成: passed={all_passed}, iteration={new_iteration}, " f"反馈={feedback[:80] if feedback else '(无)'}")
...
def revise_node(state: KBState) -> dict[str, Any]:
    ...
    try:
        ...
        tokens = usage.total_tokens
        cost_tracker = _merge_cost(dict(state.get("cost_tracker", {})), tokens, provider, "revise")
