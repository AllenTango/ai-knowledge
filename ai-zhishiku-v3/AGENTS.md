...
│   ├── collector.py        # 采集 Agent —— 从 GitHub / RSS 抓取数据
│   ├── analyzer.py         # 分析 Agent —— AI 摘要、标签、评分
│   ├── organizer.py        # 整理 Agent —— 去重、校验、归档
│   └── reviewer.py         # 审核 Agent —— 5 维质量评分
├── pipeline/
│   └── model_client.py     # 转发层（复用 workflows/model_client.py）
├── knowledge/
...
│   └── skills/
├── scheduler/
│   └── run_pipeline.ps1    # 定时调度脚本
├── hw_router/              # router.py 测试与设计解释
├── hw_supervisor/          # supervisor.py 测试与设计解释
├── logs/
├── AGENTS.md
├── ARCHITECTURE.md
├── opencode.json
